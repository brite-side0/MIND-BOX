...
Browser wallet connection and signing issues are common when first using MindBox. This guide covers the problems you may encounter when connecting a Stellar wallet (Freighter) to the web app and how to resolve them.
...
3. **Correct network** — Freighter must be set to **Stellar Testnet** to match MindBox's demo deployment.
...
2. After installation, reload the MindBox page so the `freighterApi` global becomes available.
3. Click "Connect wallet" again.

**Note:** MindBox plans to support other Stellar wallets (xBull, Albedo) via `@creit.tech/stellar-wallets-kit`. For now, Freighter is the primary supported browser wallet.
...
**Cause:** Freighter is set to **Mainnet** (Public Global Network) but MindBox runs on **Testnet**. The network passphrase baked into the transaction does not match what Freighter expects.
...
3. Switch to **"Testnet"**.
4. Retry the signing action in MindBox.

To verify the correct network after switching, connect your wallet and check the address appears in MindBox's header. If you still see network errors, confirm the server's `NETWORK` env matches (see [server env docs](server-env.md)).
...
4. If the popup is blocked by the browser, allow popups from the MindBox site (see [Browser permission issues](#popup-blocked-or-browser-permission-issues) below).
...
2. Select **"Always allow popups from app.mindbox.app"** (or whichever MindBox domain you are on).
3. Click "Connect wallet" again.
...
This table summarises the error messages you may see in the MindBox web UI and what they mean.
...
| Trustline      | User must add manually          | Created automatically by `mindbox_setup_wallet` |
